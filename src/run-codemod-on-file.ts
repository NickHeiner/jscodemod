...
export default async function runCodemodOnFile(
  ...
  const originalFileContents = await baseLog.logPhase(
  ...
  try {
    codemodResult = await baseLog.logPhase({phase: 'transform file', level: 'trace'}, transformFile);
  } catch (e) {
    thrownError = e;
    log.error({
